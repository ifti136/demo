...
def handle_login():
    ...
    if check_password_hash(user_data.get('password_hash'), password):
        ...
        # --- NEW: Redirect admin to admin panel ---
...
@login_required
def index():
    # --- NEW: If an admin logs in and tries to go to '/', redirect them to admin ---
    if session.get('role') == 'admin':
        return redirect(url_for('admin_panel'))
...
def get_all_data():
    ...
    # MODIFIED: Get all transactions for analytics, but they won't be displayed on history page
