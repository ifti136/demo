class CoinTrackerApp {
  ...
  updateAllUI() {
    ...
    this.updateBalanceAndGoalUI(
      ...
      this.data.goal,
      this.data.progress,
      this.data.estimated_days // Pass the new data
    );
  ...
  async addFromDashboard() {
    ...
    // --- THIS IS THE FIX ---
    const sourceEl = document.getElementById("addSource"); // This is now a <select>
    const amount = parseInt(amountEl.value);
    const source = sourceEl.options[sourceEl.selectedIndex].text; // Get text from <select>
    // --- END FIX ---
    ...
    if (result && result.success) {
      ...
      amountEl.value = ""; // Clear amount
      // We don't clear the select dropdown
      this.data = result;
      this.updateAllUI();
      this.loadHistoryPage(1);
    }
  ...
  async spendFromDashboard() {
    ...
    // --- THIS IS THE FIX ---
    const sourceEl = document.getElementById("spendCategory"); // This is now a <select>
    const amount = parseInt(amountEl.value);
    const source = sourceEl.options[sourceEl.selectedIndex].text; // Get text from <select>
    // --- END FIX ---
